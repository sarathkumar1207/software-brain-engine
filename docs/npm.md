...
2. Create the matching GitHub Release first, for example `v0.5.0`.
...
4. Only then publish the matching npm version, for example `sbe-cli@0.5.0`.

The npm package version and GitHub Release tag must match. `sbe-cli@0.5.0` downloads from `releases/download/v0.5.0/`.

Be careful with manual releases: the tag must be `v0.5.0`, not `vv0.3.0`. A double `v` release can show assets on GitHub but still break npm downloads.
...
npm versions are immutable. If `sbe-cli@0.5.0` has a wrapper bug, publish `sbe-cli@0.5.0`; do not try to republish `0.2.2`.
